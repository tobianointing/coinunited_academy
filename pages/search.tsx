...
import {
  ...
  formatDate,
  formatReadingTime,
  Difficulty as InlineDifficulty,
...
const range = (start = 1, stop: number, step = 1) =>
...
const Search = ({
  ...
  const fetchPaginatedDataDefault = (page: number) => {
    fetchPost_default(query, page, "EN")
      .then((data) => {
        if (data) {
          setArticles(data)
          setCurrentPage(page)
      ...
      <div className="bg-[#eaeaea]">
        <Container>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-x-[2.375rem] md:gap-x-0 md:space-x-8 text-sm md:text[1.25rem] md:font-semibold">
              <p className="hidden md:block">Layout</p>
              <button
                onClick={() => setMode("grid")}
                className={`${mode === "list" ? "opacity-20" : ""} hidden md:block`}
              >
                <OptimizedImage src="/img/grid.png" alt="grid" className="h-7 w-7" />
              </button>
              <button
                onClick={() => setMode("list")}
                className={mode === "grid" ? "hidden md:block opacity-20" : "hidden md:block"}
              >
                <OptimizedImage src="/img/list.png" alt="list" className="w-7 h-7" />
              </button>

              <button className="p-1 px-2 transition ease-in delay-100 rounded-md duration-400 hover:bg-black hover:text-white">
                Apply filters
              </button>
              <button className="p-1 px-2 transition ease-in delay-100 border rounded-md duration-400 hover:bg-white hover:text-black hover:border-black">
                Clear filters
              </button>
            </div>
            <div>
              <button
                ...
                className="flex items-center justify-center p-1 px-3 space-x-2 md:font-semibold transition ease-in delay-100 rounded-md duration-400 hover:bg-gray-100"
              >
                <OptimizedImage
                  src="/img/sort-down.png"
                  className={sort ? "w-2 h-2 rotate-180" : "w-2 h-2"}
                  alt="sort down"
                />
          ...
          <div className="flex items-center justify-center my-7">
            <div className="flex items-center justify-between space-x-3 font-bold text-gray-400">
              <button
                ...
                className="p-3 px-5 border-2 rounded-sm"
              >
                {"<"}
              </button>
              {pagination().map((page) => (
                <button
                  ...
                  className={
                    currentPage === page
                      ? "border-site-amber text-site-amber border-2 px-5 p-3 rounded-sm"
                      : "border-2 px-5 p-3 rounded-sm"
                  }
                >
                  {page}
                </button>
