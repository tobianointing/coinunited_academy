...
export const Tags = ({name}:{name:string}) =>{
    return (
        <button className="flex items-center justify-center p-1 px-2 space-x-2 text-sm text-gray-600 bg-gray-200 rounded-md">
            <span>{name}</span>
            <PlusIcon className="w-4 h-4 font-bold text-gray-700"/>
        </button>
    )
}
...
export const Difficulty =( {difficulty}:{difficulty?:string}) =>{
    ...
    return (
        <button className="flex items-center justify-center p-1 px-2 space-x-3 text-sm text-gray-600 border border-gray-300 rounded-md">
